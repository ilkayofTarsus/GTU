...
pthread_cond_t cond_task_copied;
pthread_barrier_t barrier;			// bariyer 
...
void add_task(const char* source, const char* destination) {
	...
	if(task_index >= task_capacity){
		pthread_cond_wait(&cond_task_copied, &mutex);				//bir önceki ödevde buranın kontrolü yapılıyor yer yoksa array büyütülüyordu
	
	}
	snprintf(tasks[task_index], (strlen(source) + strlen(destination) + 2), "%s|%s", source, destination);
	task_count++;
	task_index++;
...
char* get_task() {
	...
	pthread_cond_broadcast(&cond_task_copied);
	task_index--;
	return task;
}
...
void* copy_file(void* arg) {
	while (1) {
		...
		pthread_mutex_lock(&mutex);
		if (task_copied >= task_count && task_count != 0) {
			pthread_mutex_unlock(&mutex);
			break;
		}
		while (task_index < 0 || task_count == 0) {
			pthread_cond_wait(&cond_task_added, &mutex);
		}	
		char* task = get_task();
		task_copied++;
		pthread_mutex_unlock(&mutex);
		pthread_barrier_wait(&barrier);							// bariyer threadleri burada tutyor
...
int main(int argc, char* argv[]) {
	...
	pthread_cond_init(&cond_task_copied, NULL);
	...
	pthread_cond_destroy(&cond_task_copied);
